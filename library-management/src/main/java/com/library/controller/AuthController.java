...
// private static final long OTP_RESEND_COOLDOWN = 60 * 1000; // 1 phút chờ gửi
// lại
// private static final long OTP_EXPIRATION_TIME = 5 * 60 * 1000; // 5 phút hết
// hạn mã
...
// authService.checkAccount(loginRequest.getUsername(),
...
// if (user.getRole() == User.Role.LIBRARIAN || user.getRole() ==
// User.Role.ADMIN) {
...
// public ResponseEntity<?> registerProcess(@RequestBody RegisterRequest
// registerRequest, HttpServletResponse response) {
// RegisterResult registerResult =
// userService.addNewUser(registerRequest.getUsername(),
...
// public ResponseEntity<?> forgetPasswordProcess(@RequestBody
...
// OTP_RESEND_COOLDOWN)) {
// long waitSeconds = (OTP_RESEND_COOLDOWN - (currentTime - lastSentTime)) /
// 1000;
// return ResponseEntity
// .status(HttpStatus.BAD_REQUEST)
// .body(Map.of("message", "Vui lòng chờ " + waitSeconds + " giây trước khi gửi
// lại mã!"));
// }

// Result checkEmailResult =
// userService.checkEmail(forgotPasswordRequest.getUsername(),
...
// return ResponseEntity.ok(Map.of("message", "Mã xác minh đang được gửi vào
// email!"));
// }

// // Xác thực mã OTP trong session.
// @PostMapping("/api/auth/verify-otp")
// public ResponseEntity<?> verifyOtp(@RequestBody VerifyOtpRequest
// verifyOtpRequest, HttpServletRequest request) {
// HttpSession session = request.getSession();
...
// if (serverOtp == null || otpCreationTime == null || email == null ||
// verificationType == null) {
// return ResponseEntity
// .status(HttpStatus.BAD_REQUEST)
// .body(Map.of("message", "Mã OTP không tồn tại hoặc đã bị hủy. Vui lòng gửi
// lại mã mới!"));
// }
// long currentTime = System.currentTimeMillis();
...
// public ResponseEntity<?> resetPasswordProcess(@RequestBody
// ResetPasswordRequest resetPasswordRequest, HttpServletRequest request) {
// HttpSession session = request.getSession();
...
// if (otpVerified == null || !otpVerified || verifiedEmail == null ||
// verifiedType == null) {
// return ResponseEntity
// .status(HttpStatus.BAD_REQUEST)
// .body(Map.of("message", "Bạn chưa xác thực OTP thành công!"));
// }
// if (!"FORGOT_PASS".equals(verifiedType) ||
// !verifiedEmail.equals(resetPasswordRequest.getEmail())) {
// return ResponseEntity
// .status(HttpStatus.BAD_REQUEST)
...
// return ResponseEntity.ok(Map.of("message", "Mã xác minh đang được gửi lại vào
// email!"));
// }
