...
// FeedbackDTO.Response feedback =
// feedbackService.createFeedback(currentUser.getId(), request);
...
// public ResponseEntity<?> getMyFeedback(@AuthenticationPrincipal UserDetails
// userDetails) {
// if (userDetails == null) {
// return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("message",
// "Chưa đăng nhập"));
...
// userService.getUserByUsername(userDetails.getUsername());
// return
// ResponseEntity.ok(feedbackService.getFeedbackByUserId(currentUser.getId()));
...
// .body(Map.of("message", "Chỉ admin hoặc librarian được xem feedback của user
// khác"));
// }
...
// UserDTO currentUser =
// userService.getUserByUsername(userDetails.getUsername());
// FeedbackDTO.Response feedback = feedbackService.updateFeedback(id,
// currentUser.getId(), isStaff(currentUser), request);
...
// UserDTO currentUser =
// userService.getUserByUsername(userDetails.getUsername());
// feedbackService.deleteRatingFeedback(id, currentUser.getId(),
// isStaff(currentUser));
...
// return user.getRole() == User.Role.ADMIN || user.getRole() ==
// User.Role.LIBRARIAN;
// }
// }
