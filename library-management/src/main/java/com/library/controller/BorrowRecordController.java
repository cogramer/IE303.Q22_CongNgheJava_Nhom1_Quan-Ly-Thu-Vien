...
public class BorrowRecordController {
  ...
  private final ReservationService reservationService; // Đã bỏ comment để dùng cho Task 1
  ...
  public ResponseEntity<?> createLoanFromReservation(@RequestBody Map<String, Long> payload) {
    Long reservationId = payload.get("reservationId");

    if (reservationId == null) {
      return ResponseEntity.badRequest().body(Map.of("message", "Thiếu ID phiếu đặt giữ (reservationId)"));
    }

    try {
      // Đã bổ sung logic: Gọi hàm fulfillReservation có sẵn trong ReservationService
  ...
  public ResponseEntity<?> borrowBooks(
    ...
    try {
      List<BorrowRecordDTO> records = borrowRecordService.borrowMultipleBooks(userId, bookIds);
